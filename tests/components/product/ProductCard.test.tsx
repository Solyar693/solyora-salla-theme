...
describe('ProductCard', () => {
  it('removes an existing favorite through the same SDK hook', async () => {
    wishlistState.ids = [42, 99];
    ...
    const button = getByTestId('footer').querySelector('button')!;
    expect(button.classList.contains('s-product-card-wishlist-added')).toBe(true);
    button.click();
    expect(wishlistState.ids).toEqual([99]);
  });

  it('reflects favorite state after SDK updates and uses the current product ID', async () => {
    const { getByTestId, rerender } = render(<ProductCard product={product} />);
    await waitFor(() => expect(getByTestId('footer').querySelector('button')).toBeTruthy());
    wishlistState.ids = [99];
    rerender(<ProductCard product={{ id: 99, name: 'Another fixture' } as never} />);
    const button = getByTestId('footer').querySelector('button')!;
    expect(button.classList.contains('s-product-card-wishlist-added')).toBe(true);
    button.click();
    expect(wishlistState.ids).toEqual([]);
  });
